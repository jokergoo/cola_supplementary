...
- `ATC_cgi_anno`: when calculating ATC score for the $i^{th}$ CpG probe, only the 
  probes which have the same CpG annotation as $CpG_i$ are used. This method
...
- *CGI seas* All other CpG sites that do not belong to CpG islands or CGI shores.
  ...
  shelf and CGI sea to a same category.

Figure 1 visualizes the methylation profile for the top 5000 CpG probes
...
show distinct methylation patterns between subtypes. CpG probes by SD can
separate all six subtypes which is expected because the original subtypes were
also based on probes with top SD scores. Probes by CV can separate _G34_,
_IDH_, _K27_ and _RTK II Classic_ subtypes but not the other two. SD and MAD
extract more similar CpG probes (Figure 2) while CV tends to extract probes
with lower methylation in general.
...
```{r, fig.width = 14, fig.height = 10, fig.cap = "Figure 1. Top 5000 CpG probes under different top-value methods."}
...
```{r, fig.height = 5, fig.width = 6, out.width = "400px", fig.cap = "Figure 2. Euler diagram of the top 5000 CpG sites from different top-value methods. Top 5000 CpG sites extracted by ATC-related methods (the top three) have little overlap to those by SD/CV/MAD (the bottom three)."}
...
As already shown in Figure 1, the proportion of CpGs in each CpG categories
are different in the top 5000 probes extracted from different top-value
methods (the annotation on the left of each heatmap in Figure 1, also put
together in Figure 3B). E.g. probes in CGI have higher SD values, while the
...
As shown in Figure 3B, for the top 5000 CpGs by SD, there are only 20% of the
CpG belonging to CGI sea, while 46.2% of the probes in the complete 450K array are in
CGI sea (For the 450K methylation array, as visualized in Figure 3A, 30.4% of
...
```{r, fig.width = 12, fig.height = 5, fig.cap = "Figure 3. A) Frequency of CpGs in different categories. B) Fraction of different CpG categories for the top 5000 CpGs by different top-value methods."}
library(cowplot)
plot_grid(p1, p2, nrow = 1, labels = c("A", "B"), rel_widths = c(4, 6))
```

Similar as Figure 1, Figure 4 visualizes the top 5000 CpGs from diffferent CpG
...
Note in this section, when we mention "subtypes can be separated", we
...
```{r, fig.width = 14, fig.height = 8, fig.cap = "Figure 4A. Top 5000 CpGs from CGI, extracted by different top-value methods."}
ca = "island"
...
```{r, fig.width = 14, fig.height = 8, fig.cap = "Figure 4B. Top 5000 CpGs from CGI shores, extracted by different top-value methods."}
ca = "shore"
mat2 = mat1[ cpg_anno2 == ca, ]
if(file.exists(qq("GBM_450k_cgi_@{ca}_top_value_list.rds"))) {
    top_value_list = readRDS(qq("GBM_450k_cgi_@{ca}_top_value_list.rds"))
...
```{r, fig.width = 14, fig.height = 8, fig.cap = "Figure 4C. Top 5000 CpGs from CGI seas, extracted by different top-value methods."}
ca = "sea"
mat2 = mat1[ cpg_anno2 == ca, ]
if(file.exists(qq("GBM_450k_cgi_@{ca}_top_value_list.rds"))) {
    top_value_list = readRDS(qq("GBM_450k_cgi_@{ca}_top_value_list.rds"))
...
Since ATC is based on correlation, in Figure 5, we also scaled the methylation
...
z-score transformation (Figure 5A, right heatmap). The two-group
...
```{r, fig.width = 10, fig.height = 5, out.width = "800px", fig.cap = "Figure 5A. Heatmap of the top 5000 CpGs in CGI by ATC methods. Left heatmap: the original methylation values. Right heatmap: the z-scores."}
ca = "island"
mat2 = mat1[ cpg_anno2 == ca, ]
top_value_list = readRDS(qq("GBM_450k_cgi_@{ca}_top_value_list.rds"))
...
```{r, fig.width = 10, fig.height = 5, out.width = "800px", fig.cap = "Figure 5B. Heatmap of the top 5000 CpGs in CGI shores by ATC methods. Left heatmap: the original methylation values. Right heatmap: the z-scores."}
ca = "shore"
mat2 = mat1[ cpg_anno2 == ca, ]
top_value_list = readRDS(qq("GBM_450k_cgi_@{ca}_top_value_list.rds"))
...
```{r, fig.width = 10, fig.height = 5, out.width = "800px", fig.cap = "Figure 5C. Heatmap of the top 5000 CpGs in CGI seas by ATC methods. Left heatmap: the original methylation values. Right heatmap: the z-scores."}
ca = "sea"
mat2 = mat1[ cpg_anno2 == ca, ]
top_value_list = readRDS(qq("GBM_450k_cgi_@{ca}_top_value_list.rds"))
...
category. In Figure 6, samples are ordered by the median methylation value.
The bottom annotations show whether the sample belong to the `km` groups (the
same as in Figure 5) or the subtypes.

For methylation of CGI probes, the `km` or the subtype classifications have no
relation to the increasing methylation levels among samples (Figure 6A), while
...
```{r, fig.width = 8, fig.height = 6.1, out.width = "600px", fig.cap = "Figure 6A. Global methylation distribution in CpG islands."}
ca = "island"
mat2 = mat1[ cpg_anno2 == ca, ]
...
```{r, fig.width = 8, fig.height = 6.1, out.width = "600px", fig.cap = "Figure 6B. Global methylation distribution in CGI shores."}
ca = "shore"
mat2 = mat1[ cpg_anno2 == ca, ]
...
```{r, fig.width = 8, fig.height = 6.1, out.width = "600px", fig.cap = "Figure 6C. Global methylation distribution in CGI seas."}
ca = "sea"
mat2 = mat1[ cpg_anno2 == ca, ]
...
As shown in Figure 1 and 4, we can make the conclusion that SD selects better
...
annotation are visualized in Figure 7. 
...
```{r, fig.width = 12, fig.height = 2, out.width = "800px", fig.cap = "Figure 7. Partitions from different CpG categories."}
