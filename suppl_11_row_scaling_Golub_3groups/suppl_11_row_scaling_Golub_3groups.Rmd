...
scaled, no method generates stable partitions, as shown in Table 1 and Figure 1.
...
```{r, fig.width = 12, fig.height = 8, fig.cap = "Figure 1A. Consensus heatmaps with three groups. Matrix rows are scaled."}
collect_plots(res_list_1, k = 3, anno = anno, anno_col = anno_col)
```

#### Rows are unscaled

```{r, fig.width = 12, fig.height = 8, fig.cap = "Figure 1B. Consensus heatmaps with three groups. Matrix rows are unscaled."}
...
classifications and the annotations are visualized in Figure 2.

```{r, fig.height = 1, fig.width = 8, fig.cap = "Figure 2. Conseusus partitions from ATC:skmeans and SD:skmeans methods."}
...
In Figure 2, we can observe that both classification $C_{s, 3}$ and $C_{u, 3}$
...
```{r, fig.width = 14, fig.height = 7, fig.cap = "Figure 3. PCA plots for visualizing the two-group classification. Classification on the left plot is from $C_{s,3}$ and on the right is from $C_{u,3}$."}
...
illustrated in Figure 4. Both classifications give amount of sigature genes, which
...
```{r, fig.width = 8, fig.height = 8, out.width = "600px", fig.cap = "Figure 4A. Signature genes from C_{s,3}."}
tb1 = get_signatures(res_list_1["ATC:skmeans"], k = 3, anno = anno, anno_col = anno_col, simplify = TRUE)
```

#### Row are unscaled

```{r, fig.width = 8, fig.height = 8, out.width = "600px", fig.cap = "Figure 4B. Signature genes from C_{u,3}."}
...
A direct conclusion from Figure 5 is there are far more significant genes under
$C^{(2)}_{s,3}$ (1217 genes) than $C^{(2)}_{u,3}$ (245 genes).

```{r, fig.width = 12, fig.height = 6, fig.cap = "Figure 5. Significant genes between group 2 and 3 in the two classifications. Left: from $C^{(2)}_{s,3}$, right: from $C^{(2)}_{u,3}$."}
ind1 = cl1 != 1
...
Since Figure 5 only shows the relative expression (the scaled expression), in Figure 6,
...
```{r, fig.width = 8, fig.height = 3, fig.cap = "Figure 7. Overlap of signature genes in $C^{(2)}_{s,3}$ and $C^{(2)}_{u,3}$."}
...
```{r, fig.width = 12, fig.height = 7, fig.cap = "Figure 8. Gene ontology enrichment on the two sets of signature genes illustrated in Figure 5."}
